...
export function listDevNotes() {
  ...
  const rows = /** @type {import("./mappers.js").DevNoteRow[]} */ (
    database
      .prepare(
        `
        SELECT
...
export function getDevNoteById(input) {
  ...
  const row = /** @type {import("./mappers.js").DevNoteRow | undefined} */ (
    database
      .prepare(
        `
        SELECT
    ...
    database
      .prepare(
        ...
        SELECT
          ...
          updated_at AS updatedAt
        FROM dev_notes
        WHERE name COLLATE NOCASE LIKE @textPattern ESCAPE char(92)
        ORDER BY id ASC
      `,
      )
      .all({
...
/**
 * Replaces a dev-note in the dev-notes SQLite file database. (PUT)
 *
...
/**
 * Updates a dev-note in the dev-notes SQLite file database. (PATCH)
 *
...
/**
 * Deletes a dev-note in the dev-notes SQLite file database. (DELETE)
 *
